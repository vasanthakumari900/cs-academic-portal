    ...
    <>
      ...
      <motion.aside
        ...
        className={classNames(
          "fixed inset-y-0 left-0 z-40 shrink-0 border-r border-white/10 bg-[#0D9488] shadow-md transition-transform duration-300 lg:static lg:translate-x-0",
          open ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="flex h-20 items-center gap-3.5 border-b border-white/15 px-6">
          <Link to={getHomeTarget()} className="flex items-center gap-3 cursor-pointer min-w-0">
            <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#D97706] text-white text-base font-black shadow-sm font-mono shrink-0">DV</span>
            <div className="flex flex-col leading-tight min-w-0">
              <span className="text-base font-extrabold text-white tracking-wide font-mono truncate">DDGDVC</span>
              <span className="text-xs text-teal-100 mt-0.5 font-medium truncate">CS Academic Portal</span>
            </div>
          </Link>
        </div>
            ...
            <NavLink key={to} to={to} end={to.endsWith("dashboard")} onClick={onClose}
              className={({ isActive }) =>
                classNames(
                  ...
                  isActive
                    ? "bg-white/15 text-white shadow-sm border-l-4 border-[#D97706] font-bold pl-3"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-white/15 bg-[#0D9488]">
          <Link to={getHomeTarget()}
            className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-[clamp(0.65rem,1.5vw,0.75rem)] font-semibold text-teal-100 hover:text-white hover:bg-white/10 transition-all cursor-pointer truncate"
          >
