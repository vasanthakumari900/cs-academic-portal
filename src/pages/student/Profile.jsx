...
export default function Profile() {
  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-4 text-left">
      <h2 className="mb-6 font-sans text-2xl font-bold text-[#1F2937]">Profile</h2>
      <div className="bg-white border border-[#E5E7EB] rounded-xl p-6 shadow-sm">
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            <h3 className="font-sans text-lg font-semibold text-[#1F2937]">{profile?.name}</h3>
        ...
        <dl className="space-y-3 text-sm">
          <div className="flex justify-between border-b border-[#E5E7EB] pb-2">
            ...
            <dd className="text-[#1F2937] font-medium">{user?.email}</dd>
          </div>
          <div className="flex justify-between border-b border-[#E5E7EB] pb-2">
            <dt className="text-[#6B7280]">Bookmarks</dt>
            <dd className="text-[#1F2937] font-medium">{profile?.bookmarks?.length ?? 0}</dd>
          </div>
        </dl>
      </div>
    </div>
