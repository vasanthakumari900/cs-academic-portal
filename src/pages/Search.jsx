  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8 bg-[#F8FAFC]">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-12 text-center">
        ...
        <h1 className="mb-2 font-sans text-4xl font-bold text-[#0F4C81]">Global Search</h1>
        <p className="text-sm text-[#6B7280]">Search across videos, notes, question papers and placements</p>
      </motion.div>

      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <form onSubmit={handleSubmit} className="relative mx-auto max-w-2xl">
          <div className="relative overflow-hidden rounded-xl border border-[#E5E7EB] bg-white transition-all duration-300 focus-within:shadow-sm focus-within:border-[#1E88E5]/40">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)}
              ...
              className="w-full bg-transparent py-4 pl-14 pr-4 text-base outline-none text-[#0F4C81] placeholder:text-slate-400"
            />
          </div>
      ...
      {!loading && results && total > 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-10 space-y-12">
          <p className="text-sm text-[#6B7280]">
            <span className="font-semibold text-[#0F4C81]">{total}</span> result{total !== 1 ? "s" : ""} for &ldquo;<span className="font-semibold text-[#4B5563]">{q}</span>&rdquo;
          </p>
              ...
              <motion.section key={key} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-left">
                <div className="mb-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="font-sans text-lg font-bold text-[#0F4C81]">{label}</h2>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {items.slice(0, 6).map((item) => (
                    <div key={item.id}
                      className="group relative overflow-hidden rounded-xl border border-[#E5E7EB] bg-white p-4 transition-all duration-300 hover:shadow-sm hover:border-[#1E88E5]/30 text-left"
                    >
                      <h3 className="font-sans text-sm font-bold text-[#0F4C81] group-hover:text-[#1E88E5] transition-colors">{item.title || item.companyName}</h3>
